...
CategoricalAttributes = ['ZIPCode','Education Level','Family size','Does the customer have a securities account with the bank?','Does the customer have a certificate of deposit (CD) account with the bank?','Does the customer use internet banking facilities?','Does the customer uses a credit card issued by UniversalBank?']
...
#    if int(val.at[int(index),'class']) == 1: 
#        NoOfPos += 1
#    else:
#        NoOfNeg += 1
#print(NoOfPos)
...
#classZeroData['ZIPCode'].hist(bins=100,density=False)
